...
__global__ void softmax(float *in, float *out, int rows, int cols) {
  ...
  if (tid < rows) {
    ...
    for (int i = 0; i < cols; i++) {
      max_val = max(max_val, in[tid * cols + i]);
    }

    // Step 2: Compute exponentials and sum
    float sum = 0.0f;
    for (int i = 0; i < cols; i++) {
      float exp_val = expf(in[tid * cols + i] - max_val);
      out[tid * cols + i] = exp_val;
      sum += exp_val;
    }
...
int main() {
  ...
  dim3 num_blocks((N + num_threads.x - 1) / num_threads.x,
                  (N + num_threads.y - 1) / num_threads.y);
  ...
  scale_kernel<<<total_blocks, total_threads>>>(S_d, scale,
                                                N * N); // S / sqrt(D) -> S
